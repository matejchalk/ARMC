...
namespace Microsoft.Automata
{
    ...
    public class Automaton<T> : IAutomaton<T>
    {
        ...
        static Automaton<T> MkProduct(Automaton<T> a, Automaton<T> b, IBooleanAlgebraPositive<T> solver, int timeout)
        {
            long timeoutLimit;
            if (timeout > 0)
                timeoutLimit = Utilities.HighTimer.Now + Microsoft.Automata.Utilities.HighTimer.Frequency * ((long)timeout / (long)1000);
            else
                timeoutLimit = 0;
	    ...
	    */
        ...
        static public Automaton<T> MkDifference(Automaton<T> A, Automaton<T> B, int timeout)
        {
            ...
            long timeoutLimit;
            if (timeout > 0)
                timeoutLimit = Utilities.HighTimer.Now + Utilities.HighTimer.Frequency * ((long)timeout / (long)1000);
            else
                timeoutLimit = 0;
